...
//try {
//GestorJDBC db = new GestorJDBC();
//Connection conn = db.connetion();
//System.out.println("Conexion de mysql creada");
//} catch (SQLException e) {
//    showAlertDialogInf("Information", "Fail connection to server.\n\nCheck network or credenditals server.");
//showMissage("No network connection!");
//    e.printStackTrace();
//}

 /*
    private class LoadMoreData extends AsyncTask<Void, Void, ResultSet> {
        ...
        protected void onPostExecute(ResultSet rs) {
            ...
            try {
                if (rs != null) {
                    while (rs.next()) {
                        Product p = new Product(rs.getInt(1),
                                ...
                                rs.getInt(4),
                                rs.getString(5),
                                Boolean.parseBoolean(String.valueOf(rs.getInt(6))));
                        productsList.add(p);
                        moreItems++;
                    }
                    if (moreItems > 0) {
                        ...
                        Toast.makeText(ListViewProductsJDBC.this, "No more data", Toast.LENGTH_LONG).show();
                    }
                } else {
                    Toast.makeText(ListViewProductsJDBC.this, "Error en load Data", Toast.LENGTH_LONG).show();
                }
                ...
                // Text Info List
                tvInfoList.setText("Displaying " + productsList.size() + " of " + countProducts );


                System.out.println("nuevo offset " + productsList.size());
                System.out.println("lista de productos " + productsList.size());
